...
import os 
os.environ['LD_LIBRARY_PATH'] = '/usr/local/openmpi/4.1.1/gnu/11.2.0/lib:' + os.environ.get('LD_LIBRARY_PATH', '')
...
class CICalculator(FileIOCalculator):
    ...
    command = "~/bin/run2_orca5.0.3.sh orca_engrad.in"
    ...
    def __init__(
        ...
        self.slow_increase = slow_increase 
        ...
        if self.program == 'ORCA':
            CICalculator.command = " ~/bin/run2_orca5.0.3.sh orca_engrad.in"
    ...
    def write_results(self):
        ...
        # I will not say which computer, but it doesnt work this way. This is why it is done manually:
        # np.savetxt('ener.dat', energies)
        # np.savetxt('engrad0.dat', engrad_0)
        # np.savetxt('engrad1.dat', engrad_1)
        
        with open('ener.dat', 'w') as f:
            f.write(f'{energies[0]} {energies[1]}')
 
        with open('engrad0.dat', 'w') as f:
            for line in np.copy(engrad_0).reshape([-1,3]):
                f.write(f'{line[0]} {line[1]} {line[2]}\n')

        with open('engrad1.dat', 'w') as f:
            for line in np.copy(engrad_1).reshape([-1,3]):
                f.write(f'{line[0]} {line[1]} {line[2]}\n')
        ...
        with open('iteration.dat', 'w') as f:
    ...
    def penalty_results(self):
        ...
        en1, en2 = np.loadtxt("enerergies.dat")[-1][0:1]
    ...
    def ubp_forces(self):
        ...
        iteration = int(np.loadtxt('iteration.dat'))
        if iteration == 1:
            en1, en2, _ = np.loadtxt("energies.dat")
        else:
            enerline = np.loadtxt("energies.dat")[-1]
            # print('################################')
            # print(enerline)
            # print('################################')
        ...
        try:
            iteration = len(np.loadtxt('energies.dat').T[0])
        except:
            iteration = 0 


        # if slow_start is requested, the first iterations will be smoother
        iter_weight = 1

        # if slow_increase is selected, then increase smoothly:
        if self.slow_start and iteration < 10:
            iter_weight = 0.1

        # if slow_increase and slow_start, make so that increase starts at 0.3:
        elif self.slow_increase and self.slow_start and iteration > 10:
            iter_weight = min([1, 0.3 + 0.05 * (iteration-10)])

        # if only slow_increase, make so that increase starts at 0.3:
        elif self.slow_increase and not self.slow_start:
            iter_weight = min([1, 0.3 + 0.05 * (iteration)])
        
        else:
            iter_weight = 1

        # verbose if scaling is considered
        if iter_weight < 0.99:
            print(f'Slow start requested. Geff is rescaled at this step by {iter_weight:0.2f}')

        total_gradient = iter_weight * g_diff.reshape(-1) + P @ mean_grad.reshape(-1)
        total_gradient = total_gradient.reshape([-1,3])
