...
The functionality of `cris.py` is:

```
  -h, --help            show this help message and exit

Core data:
  -molcas_file MOLCAS_FILE
                        Molcas .out file with gradients and cdv.
  -g0 G0                Lower state gradient file.
  -g1 G1                Higher state gradient file.
  -cdv CDV              cdv vector file.

Vector visualization and geometry displacement:
  -ref_xyz REF_XYZ      Reference xyz file for forces and displacement.
  --generate_forces     Generate xyz file with x, y, and -s_ab vectors for visualization.
  --generate_displacements
                        Generate displaced geometries in x, -x, y, -y -s_ab and mirrored -s_ab. A folder will be generated with these geometries.
  -disp_mod DISP_MOD    Displacement module of the vectors to generate displaced geometries. Default is 0.1.
...
Default run of `cris.py` with the required `g0`, `g1`, and `cdv` returns the previous shown output. Some nice additions to this are:
- Using a reference `ref.xyz` it is possible to plot the direction of the branching plane vectors, allowing to visually observe the directions that break degeneracy. This works when plotted in [JMOL](https://jmol.sourceforge.net/) by using `jmol vectors.xyz`. Some different geometries will be shown (all the same but with different forces) which correspond to the directions of interest: no direction, $x$, $-x$, $y$, $-y$, the two minimum directions in the lower PES, projection of the tilt over the branching plane, and lower PES gradient direction.
- By plotting using `--plot_polar`, it is possible to see the conical intersection in polar coordinates. This combined with the previous step allows to see visually the directions after the conical intersection.
- By using the `--interactive` keyword, it is possible to see in a browser a 3D representation of the intersection.
- By using `--generate_displacements` it is possible to generate displaced geometries from the conical intersection geometry using the displacement vectors prevously shown. This is particularly useful, for example, when we want to follow the path in both sides of a bifurcating conical intersection. In that case, we just reoptimize in the lower state PES the geometries generated by displacement. 
